...
if [ -n "${SW_AGENT_ENABLE}" ]; then
  if [ ! -n "${SW_SERVICE_NAME}" ]; then
    SW_SERVICE_NAME="perf-redis-spring"
  fi
...
java $AGENT_OPT $JAVA_OPT $JVM_OPT -Dlog4j.configurationFile=conf/log4j2.xml -classpath /opt/perf/lib/*:/opt/perf/perf-redis-spring.jar:/opt/perf/conf/* com.github.perftool.pulsar.io.Main >>/opt/perf/logs/stdout.log 2>>/opt/perf/logs/stderr.log
